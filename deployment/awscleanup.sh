...
originAccessIdentity=$(aws cloudfront list-cloud-front-origin-access-identities \
    | jq -r '.CloudFrontOriginAccessIdentityList.Items | .[] | select(.Comment == "toh-lagom") | .Id')
...
until [[ -z $(aws rds delete-db-cluster --db-cluster-identifier toh-lagom --skip-final-snapshot) ]]
do
    sleep 10
done
